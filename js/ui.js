...
function showEeveeChoice(pokemon) {
  return new Promise(resolve => {
    ...
    const eeveeOpts = (typeof state !== 'undefined' && state.generation === 2)
  ? EEVEE_EVOLUTIONS_GEN2
  : EEVEE_EVOLUTIONS;
for (const evoData of eeveeOpts) {
...
if (pokemon.speciesId === 133) {
  const eeveeLevel = (state.generation === 2) ? 25 : 36;
  if (pokemon.level < eeveeLevel) continue;

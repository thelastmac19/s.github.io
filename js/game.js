...
function showMapScreen() {
  ...
  const gymList = state.generation === 2 ? GYM_LEADERS_GEN2 : GYM_LEADERS;
  const mapInfo = document.getElementById('map-info');
  if (mapInfo) {
    const isFinal = state.currentMap === 8;
	const leader = isFinal ? null : gymList[state.currentMap];
    mapInfo.innerHTML = isFinal
      ? `<span>Elite Four & Champion</span>`
      : `<span>Map ${state.currentMap+1}: vs <b>${leader.name}</b> (${leader.type})</span>`;
  }
  const BASE = state.generation === 2 ? './sprites/badges/gen2_' : './sprites/badges/';
  const badgeHtml = Array.from({ length: 8 }, (_, i) => {
    const earned = i < state.badges;
    const label = gymList[i].badge;
    const src = state.generation === 2 ? `${BASE}${i + 1}.png` : `${BASE}${i + 1}.png`;
    return earned
      ? `<img src="${src}" alt="${label}" title="${label}" class="badge-icon-img">`
...
function showBadgeScreen(leader) {
  ...
  if (badgeImg) {
    const badgePrefix = state.generation === 2 ? './sprites/badges/gen2_' : './sprites/badges/';
    badgeImg.src = `${badgePrefix}${state.badges}.png`;
  }
